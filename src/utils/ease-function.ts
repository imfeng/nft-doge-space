...
export function useEaseFunction(
  callback: (t: number) => any,
  startValue: number, amountOfChange: number,
  duration: number,
  easeFunction: (elapsed: number, initialValue: number, amountOfChange: number, duration: number) => number = easeInCubic) {
//   const startValue = 0;
//   const amountOfChange = 1;

  const startTime = ref(Date.now());
  const resolveFn = ref<(value: number) => void>(fn);

  function tick() {
    const elapsed = Date.now() - startTime.value;
    callback(easeFunction(elapsed, startValue, amountOfChange, duration));
